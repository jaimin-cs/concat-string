...
const Service: React.FC<Props> = ({ data }) => {
  ...
  const techNode = data?.technology?.technologiesSettings;
            ...
            <div className="flex items-start gap-[60px] 2xl:flex-row xl:flex-row lg:flex-row md:flex-col sm:flex-col flex-col">
              <div className="service-tab flex flex-col gap-[20px] 2xl:w-[470px] xl:w-[470px] lg:w-[470px] md:w-full sm:w-full w-full">
                {features.map((feature: any, idx: number) => (
                  <button
                    ...
                    className={`group p-[1px] border-[1px] rounded-[10px] transition-all duration-300 ${
                      activeTabIdx === idx ? "" : "border-[#d1d1d1]"
                    }`}
                    style={
                      activeTabIdx === idx
                        ? {
                  ...
                  >
                    <div
                      className={`rounded-[10px] w-full h-full py-[20px] px-[30px] flex items-start justify-start transition-all duration-300 ${
                        activeTabIdx === idx ? "bg-transparent" : "bg-white"
                      }`}
                    >
                      <span
                        className={`font-denton font-bold text-[22px] leading-[34px] text-left transition-all duration-300 ${
              ...
              <div className="service-tab-contents 2xl:w-[calc(100%-530px)] xl:w-[calc(100%-530px)] lg:w-[calc(100%-530px)] md:w-full sm:w-full w-full">
                {features.map((feature: any, idx: number) => (
                  <div
                    key={feature?.featureTitle || idx}
                    className={`service-tab-content rounded-[10px] border border-white shadow-custom 2xl:p-[50px] xl:p-[50px] lg:p-[40px] md:p-[40px] sm:p-[30px] p-[20px] flex flex-col items-start bg-[#D9D9D94D] h-[470px] overflow-y-auto scrollbar-hide ${
                      activeTabIdx === idx ? "" : "hidden"
                    }`}
