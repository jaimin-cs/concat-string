...
const ClientFeedback = () => {
  const { loading, error, data } = useQuery(GET_CLIENT_FEEDBACK_SERVICES);

  if (loading) return <div>Loading...</div>;
  if (error) return <div>Error loading client feedback</div>;
  ...
  const clientFeedback =
    clientFeedbackLayout?.clientFeedback?.nodes?.[0]?.testimonialSettings;
