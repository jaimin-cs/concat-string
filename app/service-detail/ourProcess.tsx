...
const OurProcess = () => {
  const { data } = useQuery(GET_SERVICE_PROCESS_FLOW, {
    variables: { id: SERVICE_PAGE_ID },
  });

  const processData =
    data?.page?.flexibleContent?.flexibleContent?.[0]?.technologies?.nodes?.[0]
      ?.technologiesSettings;
  const processTitle = processData?.ourProcessTitle || "Our Process";
          ...
          {processFlow.map((step: any, idx: number) => {
            ...
            return (
              <div
                ...
                className={`process-wrap flex flex-col ${isEven ? "items-end justify-end" : "items-start justify-start"
                  }`}
              >
                <div
                  className={`process flex gap-[15px] 2xl:items-center xl:items-center lg:items-center md:items-center sm:items-start ${isEven
                ...
                >
                  ...
                  {!isEven && (
                    <span
                      className={
                        `rounded-full w-[74px] h-[75px] flex items-center justify-center font-denton text-[36px] leading-[100%] font-bold text-white ` +
                        (step.processCount === "01"
                          ? "bg-[#0084FF]"
                  ...
                  <div
                    className={`flex flex-col items-${isEven ? "end" : "start"
                      } justify-${isEven ? "end" : "start"} ${isEven
                  ...
                  {isEven && (
                    <span
                      className={
                        `rounded-full w-[74px] h-[75px] flex items-center justify-center font-denton text-[36px] leading-[100%] font-bold text-white 2xl:order-2 xl:order-2 lg:order-2 md:order-2 sm:order-1 order-1 ` +
                        (step.processCount === "02"
                          ? "bg-[#FF9900]"
